...
(deftest test-app
 ...
 (testing "adding a weekly goal with a non exsisting parent throws an error"
   (let [uri "jdbc:postgresql://127.0.0.1:5432/goals?user=goals&password=goals"
         _ (migrate/migrate uri)
         ds (jdbc/get-datasource {:jdbcUrl uri})
         goal {:parameters 
               {:body
                {:description "Have fun doing serious side projects"
                 :level 1
                 :goal-parent (random-uuid)}}
               :ds ds}] 
    ;;  gets cought in try and catch
     (is (thrown? Exception (goals/add goal)))))
